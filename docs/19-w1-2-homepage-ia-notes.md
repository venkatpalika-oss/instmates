...
**Obsolete asset removed (CTO local review).** `public/assets/videos/avatar.mp4` had zero runtime references in the W1.2 tree (HTML, CSS, JS, manifest, service worker), so it is deleted from the tree; Git history (7e4448d) preserves it. **Accepted temporary limitation:** the HeyGen watermark stays as-is (no crop, mask or blur); a future clean clip must come from a legitimate replacement source, not watermark removal. Successful autoplay is not a release requirement; the poster is always a complete fallback.
